...
public class ResourceLookup {
	 ...
	 *
	 * At this point the method works for sources and headers (no other content
	 * types), only. This is done to use less memory and can be changed if
	 * necessary. For linked resource files, the name of the link target is
	 * relevant.
	 *
	 * @param locationSuffix the suffix to match, always used as relative path.
	 * @param projects       the projects to search
	 * @param ignoreCase     whether or not to ignore case when comparing the
	 *                       suffix.
	 */
	...
	/**
	 * Uses a lookup-tree that finds resources for locations using the canonical
	...
	/**
	 * Uses a lookup-tree that finds resources for locations using the canonical
	 * representation of the path. The method does not work for files where the name
	 * (last segment) of the resources differs from the name of the location.
	 */
	 ...
	 * from the result. Files form the first project are preferred, see
	 * {@link FileRelevance}.
	 *
	 * @param location         an URI for the location of the files to search for.
	 * @param preferredProject a project to be preferred over others, or
	 *                         <code>null</code>.
	 * @return a file for the location in one of the given projects, or
	 *         <code>null</code>. NB the returned IFile may not exist
	 */
	 ...
	 * from the result. Files form the preferred project are preferred, see
	 * {@link FileRelevance}.
	 *
	 * @param location         a path for the location of the files to search for.
	 * @param preferredProject a project to be preferred over others, or
	 *                         <code>null</code>.
	 * @return a file for the location or <code>null</code>. NB the returned IFile
	 *         may not exist
	 */
	...
	/**
	 * Iterates through a list of 'file' resources, and selects the one with the
	 * highest "relevance score".
	 *
	 * NOTE: To compute the "relevance scores" this method may cause additional
	 * project-descriptions to load. To avoid the expense of loading additional
	 * project-descriptions, we first perform a quick first-pass through the list of
	 * IFiles (which would normally be a very small list), to see if any of them is
	 * in the preferred project. In other words, if we know that the file within the
	 * preferred project is the one that's most relevant, then first try to find it
	 * directly - before getting to the more expensive loop of computing the
	 * "relevance scores" for all the files.
	 */
	private static IFile selectFile(IFile[] files, IProject preferredProject, Object originalLocation) {
		...
		/*
		 * FIX for Bug 405511: Try to find the file within the preferred project first -
		 * we want to avoid reaching the next for-loop - that loop is expensive as it
		 * might cause the loading of unnecessary project-descriptions.
		 */
	...
	/**
	 * Sorts files by relevance for Corrosion, by the criteria listed below. The
	 * most relevant files is listed first. <br>
	 * Accessible files <br>
	 * Files of preferred project <br>
	 */
	public static void sortFilesByRelevance(IFile[] filesToSort, final IProject preferredProject) {
		Collections.sort(Arrays.asList(filesToSort), (f1, f2) -> {
